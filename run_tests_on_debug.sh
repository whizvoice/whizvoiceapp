...
run_integration_tests_with_logcat() {
    ...
    # When VERBOSE_LOGGING is enabled, also capture WhizServerRepo Info logs for WebSocket debugging
    {
        if [[ "$VERBOSE_LOGGING" == "true" ]]; then
            ...
            adb logcat -v time '*:E' '*:W' 'TestRunner:V' 'com.example.whiz*:V' 'WhizServerRepo:V' 'ToolExecutor:V' 'AndroidRuntime:V' 'TTSManager:V' 'ChatViewModel:V' 'BubbleOverlayService:V' 'VoiceManager:V' 'ScreenAgentTools:V' 'BaseIntegrationTest:V' 'WebSocketReconnectionTest:V' 'ComposeTestHelper:V' 'ChatScreen:V' 'AppLifecycleTest:V' >> test_logcat_output.log 2>&1 &
        else
            adb logcat -v time '*:E' '*:W' 'TestRunner:V' 'com.example.whiz*:V' 'ToolExecutor:V' 'AndroidRuntime:V' 'TTSManager:V' 'ChatViewModel:V' 'BubbleOverlayService:V' 'VoiceManager:V' 'ScreenAgentTools:V' 'BaseIntegrationTest:V' 'WebSocketReconnectionTest:V' 'ComposeTestHelper:V' 'ChatScreen:V' 'AppLifecycleTest:V' >> test_logcat_output.log 2>&1 &
        fi
        local logcat_pid=$!
    }
