...
object AccessibilityDumpUtil {
    ...
    fun dumpNodeRecursive(node: AccessibilityNodeInfo, sb: StringBuilder, depth: Int) {
        ...
        sb.appendLine("$indent[$className] id=$resourceId text=\"$text\" desc=\"$contentDesc\" bounds=$bounds clickable=${node.isClickable} longClickable=${node.isLongClickable} childCount=${node.childCount}")
